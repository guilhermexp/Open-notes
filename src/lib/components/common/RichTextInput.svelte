<script lang="ts">
	...
	// YouTube components
	import { YouTubeLinkExtension } from './RichTextInput/YouTubeLinkExtension.js';
	...
	const handleYouTubePreviewToggle = (videoId, url, from, to) => {
		const previewKey = `${videoId}-${from}`;
		
		if (showYouTubePreview[previewKey]) {
			// Hide preview
			showYouTubePreview[previewKey] = false;
			youtubePreviews = youtubePreviews.filter(p => p.key !== previewKey);
		} else {
			// Show preview
			showYouTubePreview[previewKey] = true;
			
			// Check if preview already exists
			const existingPreview = youtubePreviews.find(p => p.key === previewKey);
			if (!existingPreview) {
...
<div class="relative w-full">
	...
	{#each youtubePreviews as preview (preview.key)}
		{#if showYouTubePreview[preview.key]}
			<div class="mt-2 mb-2 youtube-preview-container" data-preview-key={preview.key}>
				<YouTubePlayer 
